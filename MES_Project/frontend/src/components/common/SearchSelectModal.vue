<!-- src/components/common/SearchSelectModal.vue -->
<script setup>
...
const selectedRow = computed(() => props.rows.find((r) => r[props.rowKey] === selectedKey.value) || null);

const close = () => emit('update:modelValue', false);
const onSearch = () => emit('search', keyword.value);

const onSelectRow = (row) => {
    selectedKey.value = selectedKey.value === row[props.rowKey] ? null : row[props.rowKey];
};
            ...
            <div class="ssm-table-wrap">
                <table class="ssm-table">
                    ...
                    <tbody>
                        ...
                        <tr v-for="row in rows" :key="row[rowKey]" class="ssm-row" @click="onSelectRow(row)">
                            <td class="ssm-center">
                                <input type="checkbox" :checked="row[rowKey] === selectedKey" @change.stop="onSelectRow(row)" />
                            </td>
