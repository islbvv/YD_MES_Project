...
import inboundApi from '@/api/inbound'; // [변경] API 모듈 import
...
const materialList = ref([]); // 빈 배열로 초기화 (API로 채움)
...
// ------------------------------------------------------------------
// [Functions] API 호출 및 로직
// ------------------------------------------------------------------

// 1. 재고 목록 조회
const search = async () => {
    loading.value = true;
    selected.value = null; // 검색 시 상세 선택 초기화
    ...
    try {
        ...
        if (materialList.value.length === 0) {
            toast.add({ severity: 'warn', summary: '결과 없음', detail: '조건에 맞는 자재가 없습니다.', life: 3000 });
        }
    } catch (error) {
        console.error('재고 조회 실패:', error);
        toast.add({ severity: 'error', summary: '오류', detail: '데이터를 불러오지 못했습니다.', life: 3000 });
    } finally {
        loading.value = false;
    }
};

// 2. 자재 상세 정보 조회 (행 선택 시)
const selectMaterial = async (data) => {
    try {
        // [변경] 백엔드 API 호출
        const response = await inboundApi.getStockDetail(data.code);
        selected.value = response.data;
    } catch (error) {
        console.error('상세 조회 실패:', error);
        toast.add({ severity: 'error', summary: '오류', detail: '상세 정보를 불러오지 못했습니다.', life: 3000 });
    }
};

// 3. 필터 초기화 (기존 함수 유지하되 버튼은 제거됨)
const resetFilters = () => {
        ...
        <div class="stock-layout-grid">
            <div class="content-card">
                ...
                <div class="filter-row mb-4">
                    ...
                    <Button label="검색" @click="search" class="mr-2" :loading="loading" />
                </div>
                ...
                <DataTable :value="materialList" :loading="loading" paginator :rows="10" class="text-sm" selectionMode="single" rowHover @rowSelect="(e) => selectMaterial(e.data)">
                    <template #empty>
                        <div class="p-4 text-center text-gray-500">
                            {{ loading ? '데이터를 불러오는 중입니다...' : '조회된 재고 목록이 없습니다.' }}
                        </div>
                    </template>
            ...
            <div v-if="selected" class="content-card">
                ...
                <div class="p-2">
                    ...
                    <DataTable :value="selected.details" size="small" class="mb-4 text-sm" rowHover>
                        <template #empty>상세 재고 정보가 없습니다.</template>
                    ...
                    <DataTable :value="selected.history" size="small" class="text-sm" rowHover>
                        <template #empty>최근 이력이 없습니다.</template>
                        <Column field="date" header="일시" />
                        <Column field="type" header="구분" />
                        <Column field="amount" header="수량" />
                        <Column field="supplier" header="공급/요청처" />
